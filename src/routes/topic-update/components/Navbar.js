...
class Navbar extends React.Component {
  ...
  render() {
    ...
    const uploadProps = {
      ...
      action: '//dev-editservice.vcg.com/image/replace?groupId=502514260&resImageId=1135938422&token=ST-187-6db513b7e4fa780b035728ea215817035',
      beforeUpload: file => {
    ...
    return (
      <div className={s.root} style={bannerUrl ? {backgroundImage: 'url('+bannerUrl+')'} : null}>
        <div className={s.header}>
          ...
          <div className={s.search}>
            <Search placeholder="专题内搜索" onSearch={value => console.log(value)} enterButton/>
          </div>
        </div>
        <div className={s.btns}>
          ...
          {bannerUrl
            ? <ButtonGroup size="small">
                <Upload {...uploadProps}><Button size="small" type="primary">重新上传</Button></Upload>
                <Button onClick={this.handleRemove}>删除</Button>
              </ButtonGroup>
            : <Upload {...uploadProps}>
              <Button size="small" type="primary" icon="file-add">上传banner</Button>
            </Upload>}
        </div>
      </div>
    );
